...
def import_pdf_to_supabase(pdf_path: Path, category: str = "pdf"):
    if not pdf_path.exists():
        raise FileNotFoundError(f"Fichier PDF introuvable : {pdf_path}")

    existing_document = get_existing_document_by_file_name(pdf_path.name)

    if existing_document:
        return {
            "status": "already_exists",
            "message": "Ce PDF existe déjà dans la base.",
            "document_id": existing_document["id"],
            "title": existing_document["title"],
            "file_name": existing_document["file_name"]
        }

    title, content_text = extract_text_from_pdf(pdf_path)

    if not content_text.strip():
        return {
            "status": "empty",
            "message": "Aucun texte extractible trouvé dans ce PDF.",
...
if __name__ == "__main__":
    files = sorted(PDFS_DIR.rglob("*.pdf"))

    print(f"Nombre de PDF trouvés : {len(files)}")
    ...
    for i, file in enumerate(files, 1):
        ...
        try:
            result = import_pdf_to_supabase(file, category=file.parent.name)
            print(result)
